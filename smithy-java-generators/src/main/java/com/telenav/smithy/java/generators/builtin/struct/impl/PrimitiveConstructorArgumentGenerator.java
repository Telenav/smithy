 ...
 * @author Tim Boudreau
 */
final class PrimitiveConstructorArgumentGenerator<S extends Shape> implements ConstructorArgumentGenerator<S> {
    ...
    public <P extends ParameterConsumer<R>, R> void generateConstructorArgument(StructureMember<? extends S> member, List<? extends ConstructorArgumentAnnotator<? super S>> annotators, ClassBuilder<?> cb, P con, ConstructorKind ck) {
        if (annotators.isEmpty()) {
            con.addArgument(member.typeName(), member.arg());
        } else {
            MultiAnnotatedArgumentBuilder<ParameterNameBuilder<TypeNameBuilder<R>>> multi
                    = con.addMultiAnnotatedArgument();
            annotators.forEach(anno -> {
                anno.generateConstructorArgumentAnnotations(member, ck, multi, cb);
            });
            multi.closeAnnotations().named(member.arg()).ofType(member.typeName());
        }
    }
